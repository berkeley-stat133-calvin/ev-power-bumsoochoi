...
# create leaflet map
renew_map = leaflet(renew_growth_map) |>
...
# save the map into a png file
# mapshot(renew_map, file = "renewable_energy_growth_map.png")
...
# create leaflet map
non_renew_map = leaflet(total_growth_map) |>
  addProviderTiles("CartoDB.Positron") |> #cartoDB.Positron is just like the background map we use as base
  addCircles(
    lng = ~lon,
    lat = ~lat,
...
# mapshot(non_renew_map, file = 'nonrenewable_energy_growth_map.png')

```

![](renewable_energy_growth_map.png)
![](nonrenewable_energy_growth_map.png)
